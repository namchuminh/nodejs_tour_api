...
class news {
    ...
    async add(req, res) {
        try{
            ...
            if(!TieuDe || !NoiDung || !MaChuyenMuc || !DuongDan){
                return res.status(400).json({ error: "Vui lòng nhập đủ thông tin tin tức!" });
            }

            if(!/^[0-9]+$/.test(MaChuyenMuc)) return res.status(400).json({ error: "Mã chuyên mục phải là một số!" });
            if(!await Categories.findOne({ where: { MaChuyenMuc } })) return res.status(400).json({ error: "Mã chuyên mục không tồn tại!" });
            ...
            if(!createdNews) return res.status(400).json({ error: "Thêm tin tức thất bại, vui lòng thử lại!" });
    ...
    async edit(req, res) {
        try{
            ...
            if (!req.file) {
                ...
                if(!updatedNews) return res.status(400).json({ error: "Cập nhật tin tức thất bại, vui lòng thử lại!" });
            }else{
                // Lấy đường dẫn lưu trữ file ảnh
                const AnhChinh = req.file.path.replace(/\\/g, "/");
    ...
    async remove(req, res) {
        try{
            ...
            if(!newsDeleted) return res.status(404).json({ error: "Xóa tin tức không thành công, vui lòng thử lại!" });
